...
public interface PayStubRepository  extends JpaRepository<PayStub, Long> {
    ...
    String valueForOne = "p.id , p.amount , p.nbr_tasks , p.bonus , p.payment_date  , p.employee_id ";
    ...
    /*SELECT p.id , p.amount , p.nbr_tasks , p.bonus , p.payment_date , p.employee_id FROM pay_stub p INNER JOIN employee e ON e.id = p.employee_id WHERE e.email = "aldrim@gmail.com";*/
